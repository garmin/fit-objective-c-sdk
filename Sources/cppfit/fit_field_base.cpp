...
{
    return GetBitsValueT<FIT_UINT32>(offset, bits);
}

FIT_SINT32 FieldBase::GetBitsSignedValue(const FIT_UINT16 offset, const FIT_UINT8 bits) const
{
    return GetBitsSignedValueT<FIT_SINT32>(offset, bits);
}

FIT_UINT64 FieldBase::GetBitsValue64(const FIT_UINT16 offset, const FIT_UINT8 bits) const
{
    return GetBitsValueT<FIT_UINT64>(offset, bits);
}

FIT_SINT64 FieldBase::GetBitsSignedValue64(const FIT_UINT16 offset, const FIT_UINT8 bits) const
{
    return GetBitsSignedValueT<FIT_SINT64>(offset, bits);
}
...
#ifdef FIT_ENABLE_LEGACY_FLOAT_CASTS
...
void FieldBase::SetFLOAT64Value(const FIT_FLOAT64 value, const FIT_UINT8 fieldArrayIndex, const FIT_UINT16 subFieldIndex)
{
    ...
    switch (GetType())
    { // Note: This checks the type of the MAIN field since data is aligned according to that type
    case FIT_BASE_TYPE_BYTE:
    case FIT_BASE_TYPE_ENUM:
    case FIT_BASE_TYPE_UINT8:
    case FIT_BASE_TYPE_UINT8Z:
...
struct FitInvalid;
template <>
struct FitInvalid<FIT_SINT32>
{
    static const FIT_SINT32 value = FIT_SINT32_INVALID;
};
template <>
struct FitInvalid<FIT_UINT32>
{
    static const FIT_UINT32 value = FIT_UINT32_INVALID;
};
template <>
struct FitInvalid<FIT_SINT64>
{
    static const FIT_SINT64 value = FIT_SINT64_INVALID;
};
template <>
struct FitInvalid<FIT_UINT64>
{
    static const FIT_UINT64 value = FIT_UINT64_INVALID;
};

template <typename TSigned>
TSigned FieldBase::GetBitsSignedValueT(const FIT_UINT16 offset, const FIT_UINT8 bits) const
{
    typedef typename std::make_unsigned<TSigned>::type TUnsigned;
    TUnsigned value;
    TSigned signedValue;

    value = GetBitsValueT<TUnsigned>(offset, bits);

    if (value == FitInvalid<TUnsigned>::value)
        return FitInvalid<TSigned>::value;

    signedValue = (TSigned)1 << (bits - 1);
...
template <typename T>
T FieldBase::GetBitsValueT(const FIT_UINT16 offset, const FIT_UINT8 bits) const
{
    T value = 0;
    ...
    while (numValueBits < bits)
    {
        ...
        else
        {
            ...
            value |= (T)(data & mask) << numValueBits;
